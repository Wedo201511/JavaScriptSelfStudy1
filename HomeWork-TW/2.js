...
const MAX_VERTS = 5;
let adjMatrix = new Array(MAX_VERTS); // 邻接矩阵
let vertexList = []; // 顶点数组
let nVerts = 0;
...
let s = [];
 ...
 class AF {
	...
	let tempList;//private ArrayList<Integer>
let counterexample;//string

	function AF(Graph graph, int start, int end) {
		this.graph = graph;
		this.start = start;
		this.end = end;
	}
	...
	// 判断连个节点是否能连通
	function isConnectable(int start, int end) {
...
function counterexample() {
	...
	function getAdjUnvisitedVertex(int v) {
	...
	function printTheStack(Stack<Integer> theStack2) {
